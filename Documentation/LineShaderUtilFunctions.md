...
vec4 getColor()
```
- **Purpose**: Retrieves the color for the current fragment.
- **Return Type**: `vec4`
...
uint getTextureId()
```
- **Purpose**: Retrieves the texture ID associated with the current fragment.
- **Return Type**: `uint`

```glsl
uint getSamplerId()
```
- **Purpose**: Retrieves the sampler ID associated with the current fragment.
- **Return Type**: `uint`
...
```
- **Purpose**: Blends the fragment color with the pointer ring color if the pointer ring effect is enabled and the fragment is within the pointer ring.
- **Return Type**: `vec4`
