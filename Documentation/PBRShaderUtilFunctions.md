...
mat4 getView()
```
- **Description**: Returns the view matrix, which transforms world coordinates to camera space. This matrix represents the camera's orientation and position.
- **Return Type**: `mat4`

```glsl
mat4 getInvView()
```
- **Description**: Returns the inverse of the view matrix, which is used to transform coordinates from camera space back to world space.
- **Return Type**: `mat4`

```glsl
vec3 getCameraPosition()
```
- **Description**: Retrieves the position of the camera in world space. This is often used for lighting calculations and view-dependent effects.
- **Return Type**: `vec3`
...
```
- **Description**: Checks if the pointer ring effect is enabled. This effect is typically used for highlighting or selection in the scene.
- **Return Type**: `bool`

```glsl
vec3 getPointerRayDirection()
```
- **Description**: Retrieves the direction of the pointer ray in world space. This is used for raycasting and interaction with the scene.
- **Return Type**: `vec3`
...
```
- **Description**: Returns the mix factor for blending the pointer ring color with the underlying color. This controls the intensity of the ring effect.
- **Return Type**: `float`

```glsl
vec3 getPointerRingColor()
```
- **Description**: Retrieves the color of the pointer ring. This color is used to blend with the underlying surface color when the ring effect is active.
- **Return Type**: `vec3`

```glsl
float getFragToPointerRayDistance()
```
- **Description**: Calculates the distance from the fragment position to the closest point on the pointer ray. This is used to determine if the fragment is within the pointer ring.
- **Return Type**: `float`
