...
mat4 getInvView()
```
- **Description**: Returns the inverse of the view matrix, useful for transforming coordinates from camera space back to world space.
- **Return Type**: `mat4`

```glsl
vec3 getCameraPosition()
```
- **Description**: Retrieves the position of the camera in world space. This is essential for lighting calculations and effects that depend on the camera's location.
- **Return Type**: `vec3`
...
```
- **Description**: Retrieves the outer distance threshold for the pointer ring effect. This defines the maximum distance from the pointer ray where the effect is applied.
- **Return Type**: `float`

```glsl
float getPointerRingInnerDistThreshold()
```
- **Description**: Retrieves the inner distance threshold for the pointer ring effect. This defines the minimum distance from the pointer ray where the effect is applied.
- **Return Type**: `float`
...
```
- **Description**: Retrieves the color of the pointer ring effect. This color is used to highlight or differentiate the selected area.
- **Return Type**: `vec3`
