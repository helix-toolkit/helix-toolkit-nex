...
```
- **Description**: Retrieves the PBR (Physically Based Rendering) properties for the current material. This function accesses the material buffer using the material ID to obtain the material's properties.
...
mat4 getInvView()
```
- **Description**: Retrieves the inverse of the view matrix. This is used to transform coordinates from camera space back to world space.
- **Return Type**: `mat4`
...
vec2 getScreenSize()
```
- **Description**: Retrieves the dimensions of the screen or viewport. This is useful for screen-space calculations.
- **Return Type**: `vec2`
...
```
- **Description**: Returns the direction of the pointer ray in world space. This is used for raycasting or interaction purposes.
- **Return Type**: `vec3`
...
```
- **Description**: Retrieves the color of the pointer ring effect. This color is used to highlight or indicate selection.
- **Return Type**: `vec3`

```glsl
float getFragToPointerRayDistance()
```
- **Description**: Calculates the distance from the fragment position to the closest point on the pointer ray. This is used to determine if a fragment is within the pointer ring.
- **Return Type**: `float`

```glsl
bool isInPointerRing()
```
- **Description**: Determines if the current fragment is within the pointer ring effect based on the calculated distance to the pointer ray.
- **Return Type**: `bool`
