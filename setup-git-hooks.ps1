...
try {
    ...
    Write-Host "[ERROR] Failed to copy hook: $_" -ForegroundColor Red
    exit 1
}

# Verify EditorConfig exists
if (Test-Path ".editorconfig") {
    Write-Host "[OK] EditorConfig found" -ForegroundColor Green
}
else {
    Write-Host "[WARNING] .editorconfig not found" -ForegroundColor Yellow
}
