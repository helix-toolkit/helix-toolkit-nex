    ...
    )
    {
        ...
        if (rayModel.Intersects(ref bound))
        {
            ...
            for (var i = octant.Start; i < octant.End; ++i)
            {
                ...
                if (HitTestSettings.EnableSmallTriangleHitTestScaling)
                {
                    if (
                        (v0 - v1).LengthSquared() < HitTestSettings.SmallTriangleEdgeLengthSquare
                        || (v1 - v2).LengthSquared() < HitTestSettings.SmallTriangleEdgeLengthSquare
                        || (v2 - v0).LengthSquared() < HitTestSettings.SmallTriangleEdgeLengthSquare
                    )
                    {
                        scaling = HitTestSettings.SmallTriangleHitTestScaling;
                ...
                )
                {
                    ...
                    if (d >= 0 && d < minDistance) // If d is NaN, the condition is false.
                    {
                        ...
                        var pointWorld = (
                            rayModel.Position + (rayModel.Direction * d)
                        ).TransformCoordinate(ref modelMatrix);
                        result.PointHit = pointWorld;
                        result.Distance = (rayWS.Position - pointWorld).Length();

                        var p0 = Positions[t1].TransformCoordinate(ref modelMatrix);
                        var p1 = Positions[t2].TransformCoordinate(ref modelMatrix);
                        var p2 = Positions[t3].TransformCoordinate(ref modelMatrix);
        ...
        if (!BoxDisjointSphere(octant.Bound, ref sphere))
        {
            ...
            for (var i = octant.Start; i < octant.End; ++i)
            {
                if (!BoxDisjointSphere(Objects[i].Value, ref sphere))
                {
                    ...
                    var v1 = Positions[t2];
                    var v2 = Positions[t3];
                    Collision.ClosestPointPointTriangle(
                        ref sphere.Center,
                        ref v0,
                        ref v1,
                        ref v2,
