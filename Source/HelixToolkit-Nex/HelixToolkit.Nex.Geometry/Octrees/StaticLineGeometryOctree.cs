...
public class StaticLineGeometryOctree : StaticOctree<KeyValuePair<int, BoundingBox>>
{
    ...
    public StaticLineGeometryOctree(
    ...
    )
    {
        ...
        if (rayModel.Intersects(ref bound))
        {
            ...
            for (var i = octant.Start; i < octant.End; ++i)
            {
                ...
                var t0 = v0.TransformCoordinate(ref modelMatrix);
                var t1 = v1.TransformCoordinate(ref modelMatrix);
                var rayToLineDistance = rayWS.GetRayToLineDistance(
                    ref t0,
                    ref t1,
                    out var sp,
                    out var tp,
                ...
                sp.TransformCoordinate(ref svpm, out var sp3);
                tp.TransformCoordinate(ref svpm, out var tp3);
        ...
        if (!BoxDisjointSphere(octant.Bound, ref sphere))
        {
            ...
            for (var i = octant.Start; i < octant.End; ++i)
            {
                if (!BoxDisjointSphere(Objects[i].Value, ref sphere))
                {
                    ...
                    var v0 = Positions[t1];
                    var v1 = Positions[t2];
                    var distance = MathUtil.GetPointToLineDistance2D(
                        ref sphere.Center,
                        ref v0,
                        ref v1,
                        out var cloestPoint,
