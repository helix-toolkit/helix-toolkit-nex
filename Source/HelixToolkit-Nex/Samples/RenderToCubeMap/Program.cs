...
internal class App : Application
{
    ...
    protected override void Initialize()
    {
        ...
        { // Create render pass for generate cube map
            ...
            renderToCubeMapPipelineDesc.Colors[0].Format = _ctx.GetFormat(_cubeMap);
    ...
    protected override void OnTick()
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            _genCubeMapRenderPass.Colors[0] = new RenderPass.AttachmentDesc
            {
            ...
            _genCubeMapFramebuffer.Colors[0].Texture = _cubeMap!;
            cmdBuffer.BeginRendering(
                _genCubeMapRenderPass,
                _genCubeMapFramebuffer,
                Dependencies.Empty
            );
            cmdBuffer.BindRenderPipeline(_genCubeMapPipeline);
            cmdBuffer.PushConstants(
                new PushCube { Face = (uint)i, Time = (float)DateTime.Now.TimeOfDay.TotalSeconds }
            );
