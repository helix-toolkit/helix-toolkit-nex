...
public sealed class SMAANode : RenderNode
{
    ...
    )
    {
        ...
        res.Pass.Colors[0].ClearColor = new Color4(0f, 0f, 0f, 0f);

        res.Framebuf.Colors[0].Texture = outputHandle;

        var cmdBuffer = res.CmdBuffer;
        cmdBuffer.PushDebugGroupLabel(debugName, DebugColor);
