...
/// <summary>
/// Manages line render pipelines created from <see cref="LineMaterialRegistration"/>
/// entries. Each registered line material type gets its own compiled fragment shader
/// (with the custom <c>getLineColor()</c> implementation) and a corresponding
/// <see cref="RenderPipelineHandle"/>.
/// </summary>
public interface ILineMaterialManager : IDisposable
{
    /// <summary>
    /// Gets the number of registered line material pipelines.
    /// </summary>
    int Count { get; }

    /// <summary>
    /// Gets the render pipeline handle for a given line material type.
    ...
    /// <summary>
    /// Creates pipelines for all registered line material types in <see cref="LineMaterialRegistry"/>.
    /// </summary>
...
/// <summary>
/// Creates and caches line render pipelines from <see cref="LineMaterialRegistry"/>
/// registrations. Each material type gets its own fragment shader with the custom
/// <c>getLineColor()</c> GLSL implementation injected into the line template.
/// <para>
/// All pipelines share the same vertex shader (<c>vsLine.glsl</c>) and the same
...
/// </para>
/// </summary>
public sealed class LineMaterialManager(IContext context, IShaderRepository shaderRepository)
    : ILineMaterialManager
{
    private static readonly ILogger _logger = LogManager.Create<LineMaterialManager>();
    ...
    public int CreatePipelinesFromRegistry()
    {
        ...
        var vsResult = _shaderCompiler.CompileVertexShader(
            GlslUtils.GetEmbeddedGlslShader("Line.vsLine")
        );

        if (!vsResult.Success)
        {
            _logger.LogError("Failed to compile line vertex shader: {Message}", vsResult.Errors);
            return 0;
        }
        ...
        if (!psResult.Success)
        {
            _logger.LogError("Failed to compile line fragment shader: {Message}", psResult.Errors);
            return 0;
        }
        ...
        {
            _logger.LogError(
                "Failed to load line shader modules. Vertex valid={VertexValid}, Fragment valid={FragmentValid}",
                vsModule.Valid,
                psModule.Valid
            );
            return 0;
        ...
        int created = 0;
        foreach (var registration in LineMaterialRegistry.GetAllRegistrations())
        {
            if (!CreatePipelineForMaterial(registration, vsModule, psModule))
            {
                _logger.LogError(
                    "Failed to create pipeline for line material '{Name}' (ID={Id}).",
                    registration.Name,
    ...
    {
        var fragmentSource = GlslUtils.GetEmbeddedGlslShader("Line.psLineTemplate");
    ...
    {
        return LineMaterialRegistry.GetAllRegistrations().BuildColorOutputImpl(template);
    }

    private bool CreatePipelineForMaterial(
        LineMaterialRegistration registration,
    ...
    )
    {
        var pipelineDesc = new RenderPipelineDesc
        {
            VertexShader = vs,
            FragmentShader = fs,
            DebugName = $"Line_{registration.Name}",
        ...
        {
            _logger.LogError(
                "Failed to create render pipeline for line material '{Name}'.",
                registration.Name
            );
            return false;
        }
        ...
        _logger.LogInformation(
            "Created line material pipeline: {Name} (ID={Id})",
            registration.Name,
            registration.TypeId.Id
        );
        return true;
    ...
    {
        return LineMaterialRegistry.GetTypeId(name);
    }

    /// <inheritdoc/>
    public bool TryGetMaterialId(string name, out MaterialTypeId materialId)
    {
        return LineMaterialRegistry.TryGetTypeId(name, out materialId);
    }
