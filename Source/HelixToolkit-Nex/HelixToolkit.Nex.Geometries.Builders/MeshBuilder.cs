...
public sealed class MeshBuilder
{
    ...
    /// </summary>
    private const string SourceMeshNormalsShouldNotBeNull =
        "Source mesh normals should not be null.";
    ...
    /// The circle cache.
    /// </summary>
    private static readonly ThreadLocal<Dictionary<int, IList<Vector2>>> CircleCache = new(() =>
        []
    );
    ...
    /// </summary>
    private static readonly ThreadLocal<Dictionary<int, IList<Vector2>>> ClosedCircleCache = new(
        () =>
            []
    );

    private static readonly ThreadLocal<Dictionary<Vector2, IList<Vector2>>> RectangleCache = new(
        () =>
            []
    );
    ...
    /// </summary>
    private static readonly ThreadLocal<Dictionary<int, MeshGeometry3D>> UnitSphereCache = new(() =>
        []
    );
    ...
    {
        get { return this.Normals is not null; }
        set
        {
    ...
    {
        get { return this.TextureCoordinates is not null; }
        set
        {
    ...
    public static IList<Vector2> GetCircle(int thetaDiv, bool closed = false)
    {
        ...
        if (!TryGetCircleInCache(ref cache, thetaDiv, closed, out IList<Vector2>? circle))
        {
        ...
        return circle is not null && circle.Count > 0 ? [.. circle] : [];

        static bool TryGetCircleInCache(
            ref Dictionary<int, IList<Vector2>>? cache,
            int thetaDiv,
            bool closed,
            out IList<Vector2>? circle
        )
        {
            cache = closed ? ClosedCircleCache.Value : CircleCache.Value;
    ...
    /// A circle segment.
    /// </returns>
    public static IList<Vector2> GetCircleSegment(
        int thetaDiv,
        float totalAngle = 2 * (float)Math.PI,
        float angleOffset = 0
    )
    {
        int pointNumber = thetaDiv + 1;
        var circleSegment = new Vector2Collection(pointNumber);
    ...
    {
        Vector2 cacheKey = new(width, height);
        if (!RectangleCache.Value!.TryGetValue(cacheKey, out IList<Vector2>? rectangle))
        {
            float halfWidth = width / 2f;
            float halfHeight = height / 2f;
            Vector2 topLeft = new(-halfWidth, halfHeight);
            Vector2 topRight = new(halfWidth, halfHeight);
            Vector2 bottomRight = new(halfWidth, -halfHeight);
            Vector2 bottomLeft = new(-halfWidth, -halfHeight);
    ...
    {
        if (UnitSphereCache.Value!.TryGetValue(subdivisions, out MeshGeometry3D? value))
        {
            return value;
        }
    ...
    private static void ComputeNormals(
        ...
        out IList<Vector3> normals
    )
    {
        var result = new Vector3Collection(positions.Count);
        result.Resize(positions.Count, true);
        ...
        MeshGeometryHelper.NormalizeInPlace(result);
        normals = result;
    }
    ...
    public static void ComputeTangents(
        ...
        IList<int> triangleIndices,
        out IList<Vector3> tangents,
        out IList<Vector3> bitangents
    )
    {
        positions ??= [];
        normals ??= [];
    ...
    {
        if (
            meshGeometry.Positions is not null
            && meshGeometry.Normals is not null
            && meshGeometry.TextureCoordinates is not null
            && meshGeometry.TriangleIndices is not null
        )
        {
            ComputeTangents(
    ...
    public void AddDodecahedron(Vector3 center, Vector3 forward, Vector3 up, float sideLength)
    {
        ...
        {
            var newPoint =
                baseCenter + forward * point.X * radiusFace + right * point.Y * radiusFace;
            basePoints.Add(newPoint);
            this.Positions.Add(newPoint);
        }
        // Angle of Projected Isosceles triangle
        var gamma = (float)
            Math.Acos(1 - (sideLength * sideLength / (2 * radiusSphere * radiusSphere)));
        // Base Upper Points
        foreach (var point in basePoints)
        {
            ...
            var newPoint = new Vector3(
                radiusSphere * (float)Math.Cos(gamma),
                0,
                radiusSphere * (float)Math.Sin(gamma)
            );
        ...
        {
            var newPoint =
                topCenter - forward * point.X * radiusFace + right * point.Y * radiusFace;
        ...
        if (this.TextureCoordinates != null)
        {
            for (var i = positionsCount; i < this.Positions.Count; i++)
            {
                ...
                var u = (float)
                    Math.Atan2(Vector3.Dot(planeCTP, forward), Vector3.Dot(planeCTP, right));
        ...
        // Add Faces
        // Base Polygon
        this.AddPolygonByTriangulation([
            .. this.Positions.Skip(positionsCount).Take(5).Select((p, i) => i),
        ]);
        // Top Polygon
        this.AddPolygonByTriangulation([
            .. this.Positions.Skip(positionsCount + 15).Select((p, i) => 15 + i),
        ]);
        // SidePolygons
        for (var i = 0; i < 5; i++)
        {
    ...
    {
        var positions = new Vector3[] { new(0, 0, 1), new(0, 1, 1), new(1, 1, 1), new(1, 0, 1) };
        var normals = new Vector3[] { new(0, 0, 1), new(0, 0, 1), new(0, 0, 1), new(0, 0, 1) };
        var i0 = this.Positions.Count;
        var indices = new int[] { i0 + 0, i0 + 3, i0 + 2, i0 + 0, i0 + 2, i0 + 1 };
        var texcoords = new Vector2[] { new(0, 1), new(1, 1), new(1, 0), new(0, 0) };
    ...
    public void AddFacePX()
    {
        ...
        var normals = new Vector3[] { new(1, 0, 0), new(1, 0, 0), new(1, 0, 0), new(1, 0, 0) };
    ...
    public void AddFacePY()
    {
        ...
        var normals = new Vector3[] { new(0, 1, 0), new(0, 1, 0), new(0, 1, 0), new(0, 1, 0) };
    ...
    public void AddQuads(
        IList<Vector3> quadPositions,
        IList<Vector3>? quadNormals,
        IList<Vector2>? quadTextureCoordinates
    )
    {
        if (this.Normals != null)
        {
            if (quadNormals is null)
            {
        ...
        if (this.TextureCoordinates != null)
        {
            if (quadTextureCoordinates is null)
            {
        ...
        Debug.Assert(
            quadPositions.Count > 0 && quadPositions.Count % 4 == 0,
            WrongNumberOfPositions
        );
    ...
    )
    {
        ...
        if (columns < 2 || rows < 2)
        {
            throw new InvalidOperationException("columns or rows too small");
        }

        if (width <= 0 || height <= 0)
        {
            throw new InvalidOperationException("width or height too small");
        }
        ...
        if (this.TextureCoordinates != null)
        {
            this.AddRectangularMeshTextureCoordinates(
                rows,
                columns,
                flipTexCoordsVAxis,
                flipTexCoordsUAxis
        ...
        for (int i = 0; i < rows; i++)
        {
            ...
            for (int j = 0; j < columns; j++)
            {
                ...
                Vector3 u = Vector3.Subtract(
                    this.Positions[index0 + (i1 * columns) + j0],
                    this.Positions[index0 + (i0 * columns) + j0]
                );
                Vector3 v = Vector3.Subtract(
                    this.Positions[index0 + (i0 * columns) + j1],
                    this.Positions[index0 + (i0 * columns) + j0]
                );
    ...
    /// </param>
    public (int ActualRows, int ActualColumns) AddRectangularMeshTriangleIndices(
        int index0,
        int rows,
        int columns,
        bool rowsClosed,
    ...
    /// set the flag to true to create a sphere mesh (triangles at top and bottom).
    /// </param>
    private void AddRectangularMeshTriangleIndicesFlipped(
        int index0,
        int rows,
        int columns,
        bool isSpherical = false
            ...
            for (var j = 0; j + 1 < n; j++)
            {
                ...
                if (this.TextureCoordinates != null)
                {
                    this.TextureCoordinates.Add(
                        new Vector2(
                            (float)i / (thetaDiv - 1),
                            textureValues == null ? (float)j / (n - 1) : textureValues[j]
                        )
                    );
                    this.TextureCoordinates.Add(
                        new Vector2(
                            (float)i / (thetaDiv - 1),
                            textureValues == null ? (float)(j + 1) / (n - 1) : textureValues[j + 1]
                        )
                    );
                }
    ...
    public void AddSurfaceOfRevolution(
        ...
        int thetaDiv = 36,
        IList<float>? textureValues = null
    )
    {
        if (this.TextureCoordinates != null)
        {
    ...
    )
    {
        ...
        {
            throw new ArgumentOutOfRangeException("Torus must have a Diameter bigger than 0");
        }
        ...
        else
        {
            ...
            crossSectionPoints =
            [
                .. crossSectionPoints.Select(p => new Vector2(
                    p.X * tubeDiameter * .5f,
                    p.Y * tubeDiameter * .5f
                )),
            ];
            // Transform the Cross-Section Points to 3D Space
            var crossSection3DPoints = crossSectionPoints
            ...
            for (var i = 0; i < thetaDiv; i++)
            {
                ...
                var rotatedPoints = crossSection3DPoints
                    .Select(p3D => new Vector3(
                        (float)Math.Cos(angle) * (p3D.X + torusDiameter * .5f),
                        (float)Math.Sin(angle) * (p3D.X + torusDiameter * .5f),
                        p3D.Z
                    ))
                    .ToList();
            ...
            if (this.Normals != null)
            {
                for (var i = 0; i < thetaDiv; i++)
                {
                    ...
                    var rotatedOrigin = new Vector3(
                        (float)Math.Cos(angle) * torusDiameter * .5f,
                        (float)Math.Sin(angle) * torusDiameter * .5f,
                        0
                    );
    ...
    public void AddTriangleFan(
        IList<Vector3> fanPositions,
        IList<Vector3>? fanNormals = null,
        IList<Vector2>? fanTextureCoordinates = null
    )
    {
        if (this.Normals != null)
        {
            if (fanNormals == null)
            {
                throw new InvalidOperationException(
                    "Cannot add triangle fan without normals when the mesh has normals."
                );
            }
        }
        ...
        if (this.TextureCoordinates != null)
        {
            if (fanTextureCoordinates == null)
            {
                throw new InvalidOperationException(
                    "Cannot add triangle fan without texture coordinates when the mesh has texture coordinates."
                );
            }
        }
    ...
    )
    {
        if (this.Normals != null)
        {
            ...
            {
                throw new InvalidOperationException(
                    "Cannot add triangles without normals when the mesh has normals."
                );
            }
        }
        ...
        if (this.TextureCoordinates != null)
        {
            if (triangleTextureCoordinates == null)
            {
                throw new InvalidOperationException(
                    "Cannot add triangles without texture coordinates when the mesh has texture coordinates."
                );
            }
        }
        ...
        if (
            triangleTextureCoordinates != null
            && triangleTextureCoordinates.Count != trianglePositions.Count
        )
        {
            throw new InvalidOperationException(WrongNumberOfTextureCoordinates);
        }
    ...
    )
    {
        if (this.Normals != null)
        {
            if (stripNormals == null)
            {
                throw new InvalidOperationException(
                    "Cannot add triangle strip without normals when the mesh has normals."
                );
            }
        }
        ...
        if (this.TextureCoordinates != null)
        {
            if (stripTextureCoordinates == null)
            {
                throw new InvalidOperationException(
                    "Cannot add triangle strip without texture coordinates when the mesh has texture coordinates."
                );
            }
        }
        ...
        if (
            stripTextureCoordinates != null
            && stripTextureCoordinates.Count != stripPositions.Count
        )
        {
            throw new InvalidOperationException(WrongNumberOfTextureCoordinates);
        }
    ...
    {
        this.AddTube(path, null, [diameter], thetaDiv, isTubeClosed, frontCap, backCap);
    }
    ...
    )
    {
        if (sectionXAxis is null || sectionXAxis.Equals(default))
        {
            ...
            Vector3 vecLeft1 = new(dir.Y, dir.Z, dir.X);
            if (vecLeft1 == dir)
            {
    ...
    public void AddTube(
        IList<Vector3>? path,
        IList<float>? sectionAngles,
        IList<float>? xTextureCoordinates,
        IList<float>? sectionScales,
        IList<Vector2> section,
        Vector3 sectionXAxis,
        bool isTubeClosed,
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException(WrongNumberOfScales);
        }
        ...
        Vector3 preXAxis = Vector3.Normalize(
            PlaneHelper.Create(Vector3.Zero, currentDir).Project(ref sectionXAxis)
        );
        if (isTubeClosed)
        {
            ...
            Quaternion rotateToPreDir = QuaternionHelper.BetweenDirections(
                ref currentDir,
                ref preDir
            );
        ...
        for (int i = 0; i < pathLength; i++)
        {
            ...
            float sectionScale = sectionScales is not null
                ? sectionScales[i % sectionScales.Count]
                : 1f;
            ...
            Vector3Collection newSection = CreateSectionPerpendicularToDirection(
                section,
                rotatedXAxis,
                currentP,
                currentDir,
            ...
            {
                Ray ray = new(newSection[j], -currentDir);
        ...
        var (actualRows, actualColumn) = this.AddRectangularMeshTriangleIndices(
            index0,
            rowsPath,
            sectionLength,
            isSectionClosed,
                    ...
                    for (int j = 0; j < sectionLength; j++)
                    {
                        ...
                        if (xTextureCoordinates is not null)
                        {
                            textureCoordinate = new Vector2(
                                xTextureCoordinates[i % xTextureCoordinates.Count],
                                (float)j / (sectionLength - 1)
                            );
                        }
                        this.TextureCoordinates?.Add(textureCoordinate);
                    }
        ...
        if (!isTubeClosed && (frontCap || backCap))
        {
            ...
            if (backCap)
            {
                ...
                [
                    .. Positions
                        .Skip(Positions.Count - section.Count)
                        .Take(section.Count)
                        .Reverse(),
                ];
            ...
            if (frontCap)
            {
                Vector3[] circleFront = [.. Positions.Take(section.Count)];
    ...
    /// <returns></returns>
    private static Vector3Collection CreateSectionPerpendicularToDirection(
        IList<Vector2> section,
        Vector3 sectionXAxis,
        Vector3 origin,
        Vector3 direction,
        float angle = 0,
        float sectionScale = 1
    )
    {
        int sectioCount = section.Count;
        Vector3Collection mappingSection = new(sectioCount);
    ...
    public void Append(MeshGeometry3D mesh)
    {
        this.Append(
            ...
            this.Normals is not null ? mesh.Normals : null,
            this.TextureCoordinates is not null ? mesh.TextureCoordinates : null
        );
    }
    ...
    )
    {
        ...
        if (
            textureCoordinatesToAppend is not null
            && textureCoordinatesToAppend.Count != positionsToAppend.Count
        )
        {
            throw new InvalidOperationException(WrongNumberOfTextureCoordinates);
        }
    ...
    /// </param>
    public void ChamferCorner(
        Vector3 p,
        float d,
        float eps = 1e-6f,
        IList<Vector3>? chamferPoints = null
    )
    {
    ...
    public void Scale(float scaleX, float scaleY, float scaleZ)
    {
        for (var i = 0; i < this.Positions.Count; i++)
        {
            this.Positions[i] = new Vector3(
                this.Positions[i].X * scaleX,
                this.Positions[i].Y * scaleY,
                this.Positions[i].Z * scaleZ
            );
        }
        ...
        if (this.Normals != null)
        {
            ...
            {
                var v = new Vector3(
                    this.Normals[i].X * scaleX,
                    this.Normals[i].Y * scaleY,
                    this.Normals[i].Z * scaleZ
                );
                v = Vector3.Normalize(v);
                this.Normals[i] = v;
            }
    ...
    public MeshGeometry3D ToMesh()
    {
        ...
        if (
            this.TextureCoordinates is not null
            && this.TextureCoordinates.Count != this.Positions.Count
        )
        {
            throw new InvalidOperationException(WrongNumberOfTextureCoordinates);
        }
    ...
    private static void AppendSphere(
        ...
        int thetaSteps,
        int phiSteps,
        out IList<Vector3> positions,
        out IList<Vector3> normals,
        out IList<Vector2> textureCoordinates,
        out IList<int> triangleIndices
    )
    {
