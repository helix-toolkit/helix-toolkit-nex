...
public sealed class ContourHelper
{
    ...
    /// </summary>
    private static readonly IDictionary<ContourFacetResult, int[,]> ResultIndices = new Dictionary<
        ContourFacetResult,
        int[,]
    >
    {
    ...
    public ContourHelper(Plane plane, MeshGeometry3D originalMesh)
        : this(PlaneHelper.GetPlanOrigin(ref plane), plane.Normal, originalMesh) { }
    ...
    public ContourHelper(Vector3 planeOrigin, Vector3 planeNormal, MeshGeometry3D originalMesh)
    {
        ...
        this.meshTextureCoordinates = hasTextureCoordinates
            ? originalMesh.TextureCoordinates?.ToArray()
            : null;
        ...
        this.d = -(
            (planeNormal.X * planeOrigin.X)
            + (planeNormal.Y * planeOrigin.Y)
    ...
    private static float CalculatePoint(
        float firstPoint,
        float secondPoint,
        float firstSide,
        float secondSide
    ...
    private void SetData(int index0, int index1, int index2)
    {
        ...
        this.sides[0] =
            (this.a * this.points[0].X)
            + (this.b * this.points[0].Y)
            + (this.c * this.points[0].Z)
            + this.d;
        this.sides[1] =
            (this.a * this.points[1].X)
            + (this.b * this.points[1].Y)
            + (this.c * this.points[1].Z)
            + this.d;
        this.sides[2] =
            (this.a * this.points[2].X)
            + (this.b * this.points[2].Y)
            + (this.c * this.points[2].Z)
            + this.d;
    }
