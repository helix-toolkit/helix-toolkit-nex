...
public static class SweepLinePolygonTriangulator
{
            ...
            else
            {
                ...
                while (pointStack.Count != 0)
                {
                    ...
                    if (
                        right == newPoint
                        && IsCCW(
                            new List<Vector2> { newPoint.Point, p2.Point, pointStack.Peek().Point }
                        )
                    )
                    {
                        top = pointStack.Pop();
    ...
    /// <returns></returns>
    private static List<Tuple<int, int>> CalculateDiagonals(
        List<PolygonPoint> events,
        bool sweepDown = true
    )
    {
        ...
        for (var i = 0; i < events.Count; i++)
        {
            ...
            switch (evClass)
            {
                case PolygonPointClass.Start:
                    ...
                    statusAndHelper.Add(
                        new StatusHelperElement(sweepDown ? ev.EdgeTwo : ev.EdgeOne, ev)
                    );
                    break;
    ...
    private static List<PolygonData> SplitIntoPolygons(
        PolygonData poly,
        List<Tuple<int, int>> diagonals
    )
    {
        ...
        foreach (
            var edge in poly
                .Points.Select(p => p.EdgeTwo)
                .Union(
                    diagonals.Select(d => new PolygonEdge(
                        poly.Points[d.Item1],
                        poly.Points[d.Item2]
                    ))
                )
    ...
    internal static PolygonEdge BestEdge(
        PolygonPoint point,
        PolygonEdge lastEdge,
        List<PolygonEdge> possibleEdges
    )
    {
        ...
        {
            // Next Edge Vector
            var edgeVector = Vector2.Normalize(
                possibleEdge.PointTwo!.Point - possibleEdge.PointOne!.Point
            );
