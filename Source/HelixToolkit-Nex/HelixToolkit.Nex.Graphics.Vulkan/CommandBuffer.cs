...
/// </summary>
/// <param name="context">The Vulkan context that owns this command buffer.</param>
/// <remarks>
...
internal sealed class CommandBuffer : ICommandBuffer, IDisposable
{
    ...
    )
    {
        ...
        HxDebug.Assert(
            buf is not null && buf.VkUsageFlags.HasAllFlags(VK.VK_BUFFER_USAGE_INDEX_BUFFER_BIT)
        );
    ...
    public void BindVertexBuffer(uint index, in BufferHandle buffer, size_t bufferOffset)
    {
        ...
        HxDebug.Assert(buf.VkUsageFlags.HasAllFlags(VK.VK_BUFFER_USAGE_VERTEX_BUFFER_BIT));
        unsafe
        {
    ...
    public void FillBuffer(in BufferHandle buffer, size_t bufferOffset, size_t size, size_t data)
    {
        ...
        if (buf.VkUsageFlags.HasAllFlags(VK.VK_BUFFER_USAGE_VERTEX_BUFFER_BIT))
        {
    ...
    )
    {
        ...
        unsafe
        {
            ...
            if (srcBuf.VkUsageFlags.HasAllFlags(VK.VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT))
            {
                srcStage |= VkPipelineStageFlags2.DrawIndirect;
            }
            if (srcBuf.VkUsageFlags.HasAllFlags(VK.VK_BUFFER_USAGE_VERTEX_BUFFER_BIT))
            {
            ...
            if (dstBuf.VkUsageFlags.HasAllFlags(VK.VK_BUFFER_USAGE_VERTEX_BUFFER_BIT))
            {
                dstStage |= VkPipelineStageFlags2.VertexInput;
            }
    ...
    public void TransitionToRenderingLocalRead(in TextureHandle handle)
    {
        ...
        HxDebug.Assert(img is not null && !img.IsSwapchainImage);
        HxDebug.Assert(
            img!.UsageFlags.HasAllFlags(VK.VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT),
