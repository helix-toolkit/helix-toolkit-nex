...
public class Buffer
{
    ...
    private struct DummyUniformData
    {
        public float Value1;
        public int Value2;
    }
    ...
    public void CreateStorageBuffer()
    {
        ...
        {
            Value1 = _rnd.NextFloat(-1, 1),
            Value2 = _rnd.Next(-100, 100),
        };
        BufferResource? buffer = null; // Initialize the variable to avoid CS0165
        var result = _vkContext?.CreateBuffer(
            new BufferDesc()
            {
        ...
        Assert.IsTrue(
            ...
            "Buffer creation failed with error: " + result.ToString()
        );
    ...
    public unsafe void CreateUniformBuffer()
    {
        ...
        Assert.IsTrue(
            result == ResultCode.Ok,
            "Uniform buffer creation failed with error: " + result.ToString()
        );
    ...
    public unsafe void CreateVertexBuffer(int vertexCount)
    {
        ...
        BufferResource? buffer = null; // Initialize the variable to avoid CS0165
        var result = _vkContext?.CreateBuffer(
            new BufferDesc() { DataSize = size, Usage = BufferUsageBits.Vertex },
            out buffer,
            "TestVertexBuffer"
        );
        Assert.IsTrue(
        ...
        Assert.IsTrue(
            downloaded.SequenceEqual(vertices),
            "Downloaded vertices do not match the original data."
        );
        buffer.Dispose(); // Clean up the buffer after the test
    }
    ...
    public unsafe void CreateIndexBuffer(int indexCount)
    {
        ...
        var result = _vkContext?.CreateBuffer(
            new BufferDesc() { DataSize = size, Usage = BufferUsageBits.Index },
            out buffer,
            "TestIndexBuffer"
        );
        Assert.IsTrue(
        ...
        Assert.IsTrue(
            downloaded.SequenceEqual(indices),
            "Downloaded indices do not match the original data."
        );
        buffer.Dispose(); // Clean up the buffer after the test
    }
    ...
    public void CreateIndirectBuffer()
    {
        BufferResource? buffer = null; // Initialize the variable to avoid CS0165
        var result = _vkContext?.CreateBuffer(
            new BufferDesc() { DataSize = 512, Usage = BufferUsageBits.Indirect },
            out buffer,
            "TestIndirectBuffer"
        );
        Assert.IsTrue(
            result == ResultCode.Ok,
            "Indirect buffer creation failed with error: " + result.ToString()
        );
