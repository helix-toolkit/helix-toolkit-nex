namespace HelixToolkit.Nex.Wpf.Tests;

/// <summary>
/// Tests for the WPF viewport's platform-independent lifecycle coordinator.
/// </summary>
[TestClass]
public sealed class ViewportLifecycleTests
{
    ...
    /// <summary>
    /// Verifies that replacement is ignored while the lifecycle is unloaded, so a
    /// resize notification cannot resurrect a suspended viewport.
    /// </summary>
    [TestMethod]
    public void Replace_WhileUnloaded_IsIgnored()
    {
        var creationCount = 0;
        var lifecycle = CreateLifecycle();
    ...
    /// <summary>
    /// Verifies that disposal after unloading still releases the resources retained
    /// across the suspended period.
    /// </summary>
    [TestMethod]
    public void Dispose_AfterUnload_StillRunsTerminalCleanup()
    {
        var terminalDisposeCount = 0;
        var deactivationCount = 0;
        var session = new TestSession();
        ...
        Assert.AreEqual(ViewportLifecycleState.Disposed, lifecycle.State);
        Assert.AreEqual(1, session.DisposeCount);
        Assert.AreEqual(2, deactivationCount);
        Assert.AreEqual(1, terminalDisposeCount);
    }
